...
if __name__ == "__main__":
    ...
    fn = "results/xlsx/burned_area_cwd_corr.xlsx"
    ...
    for i, region in enumerate(REGIONS):
        ...
        fn = f"results/xlsx/{region_name}/burned_area_cwd_values.xlsx"
        ...
        axt.text(0.8, 0.20, f"$n = {n}$", transform=axt.transAxes, fontsize=7)
        ...
        if p < 0.05:
            ...
            text = f"$r = {r:.2f}$"
        axt.text(0.8, 0.10, text, transform=axt.transAxes, fontsize=7)
        axt.set_title(region["name"].upper(), fontsize=8)
        axt.set_xlabel("Burned area (ha)", fontsize=8)
        axt.set_ylabel("CWD (mm)", fontsize=8)
        ...
        axb.text(0.8, 0.20, f"$n = {n}$", transform=axb.transAxes, fontsize=7)
        ...
        if p < 0.05:
            ...
            text = f"$r = {r:.2f}$"
        axb.text(0.8, 0.10, text, transform=axb.transAxes, fontsize=7)
    ...
    save_to = os.path.join(output_folder, "burned_area_cwd_corr.pdf")
